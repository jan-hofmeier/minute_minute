...
#include "nand/nand.h"
#include "nand/isfs/isfs.h"
#include "nand/isfs/volume.h"
#include "nand/isfs/super.h"
#include "nand/isfs/isfshax.h"
...
int isfs_init(void)
{
    ...
    ctx = isfs_get_volume(ISFSVOL_REDSLC);
    ctx->mounted = !isfs_load_super(ctx, 0, ISFSHAX_GENERATION_FIRST);
    if(ctx->mounted)
        _isfsdev_init(ctx);

    ctx = isfs_get_volume(ISFSVOL_REDSLCCMPT);
    ctx->mounted = !isfs_load_super(ctx, 0, 0xffffffff);
    if(ctx->mounted)
        _isfsdev_init(ctx);
